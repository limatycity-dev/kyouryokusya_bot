...
**症状：**  
- 既存クエストに message_id が保存されていない  
- embed 更新ができない  

**対処：**  
- エラーを返す（ユーザー向けは簡潔に）  
- ログに「message_id が未設定」と記録  
- 必要に応じて移行スクリプトで補完  
...
## 5.3 message fetch 失敗（最も多い）

**症状：**  
- thread.messages.fetch(message_id) が例外  
- メッセージが削除されている  
- BOT が権限不足  
