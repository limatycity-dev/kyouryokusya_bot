...
async function handleQuestCreateModal(interaction) {
    ...
    try {
        ...
        const embed = (0, quest_embed_1.createQuestEmbed)({ title, description, points, type });
        await thread.send({ embeds: [embed] });
        await client_1.db.query(`INSERT INTO quests (
        category_id, title, description, points, type, status, forum_thread_id, issuer_id
      ) VALUES ($1,$2,$3,$4,$5,'active',$6,$7)`, [categoryId, title, description, points, type, thread.id, issuerId]);
        return interaction.reply({
